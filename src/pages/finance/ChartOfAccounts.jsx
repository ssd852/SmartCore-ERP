...
const TYPES = ['أصل', 'التزام', 'حقوق ملكية', 'دخل', 'مصروف'];

function AccountForm({ row, onClose, onSave, isSaving }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form); }} className="flex flex-col gap-4">
      <div>
        <label className="block text-xs font-bold text-slate-400 mb-1.5">{t('account_name')}</label>
        <input className="erp-input" value={form.account_name} onChange={e => set('account_name', e.target.value)} required disabled={isSaving} />
      </div>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs font-bold text-slate-400 mb-1.5">{t('balance')}</label>
          <input className="erp-input text-left" dir="ltr" type="number" step="any" value={form.balance} onChange={e => set('balance', e.target.value)} required disabled={isSaving} />
        </div>
      </div>
...
export default function ChartOfAccounts() {
  ...
  const columns = [
    { key: 'account_id',   label: t('account_id') },
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: rows, error } = await supabase.from('accounts').select('*').order('account_id', { ascending: false });
  ...
  const handleSave = async (form, row, onClose) => {
    ...
    try {
      ...
      if (row?.account_id) {
        const { error } = await supabase.from('accounts').update(form).eq('account_id', row.account_id);
        if (error) throw error;
        setData(p => p.map(r => r.account_id === row.account_id ? { ...r, ...form } : r));
  ...
  const handleDelete = async (row) => {
    try {
      if (!supabaseReady) throw new Error('Supabase is not configured.');
      const { error } = await supabase.from('accounts').delete().eq('account_id', row.account_id);
      if (error) throw error;
      setData(p => p.filter(r => r.account_id !== row.account_id));
  ...
  const form = ({ row, onClose }) => (
    <AccountForm row={row} onClose={onClose} isSaving={isSaving} onSave={(f) => handleSave(f, row, onClose)} />
  );
